...
func GetPublicIPAddress() {
	...
	if err != nil {
		main.StandardPrinter(main.ErrorRedColor, "Could not get response from "+URL)
		main.StandardPrinter(main.WarningYellowColor, "Check your Internet Connection")
	...
	if err != nil {
		main.StandardPrinter(main.ErrorRedColor, "Could not find IPv4 Address")
		panic(err) //Exit upon error, below code must not be executed
	}
	main.ResultPrinter("Public IPv4 Address: ", string(IP)) //Cast []bByte to String and return
}

func GetLocalIPAddress() {
	networkInterfaces, err := net.Interfaces() //Get the name of all Network Interface Cards
	if err != nil {                            //Error Handling
		main.StandardPrinter(main.ErrorRedColor, "Unfortunately it is not possible to get your local IP")
		panic(err) //Exit upon error, below code must not be executed
	}
	main.ResultPrinter("Local IP Address:", "")
	for _, networkInterface := range networkInterfaces { //Iterate over each Network Interface Card
		main.StandardPrinter(main.BoldWhite, "\t["+string(networkInterface.Name)+"] "+networkInterface.HardwareAddr.String()) // Print Name and Hardware Address
		...
		if err == nil {
			for _, interfaceAddress := range interfaceAddresses {
				if strings.Contains(interfaceAddress.String(), ".") { //Check for IPv4 Address
					main.StandardPrinter(main.None, "\t\t"+"IPv4"+" -> "+interfaceAddress.String())
				} else if strings.Contains(interfaceAddress.String(), ":") { //Check for IPv6 Address
					main.StandardPrinter(main.None, "\t\t"+"IPv6"+" -> "+interfaceAddress.String())
				}
			}
		}
	}
